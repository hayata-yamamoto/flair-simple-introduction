...
def main() -> None:
    ...
    x_train = np.array([embed_by_model(x, doc_embeddings) for x in tqdm(x_train)])
    x_test = np.array([embed_by_model(x, doc_embeddings) for x in tqdm(x_test)])

    clf = neural_network.MLPClassifier(hidden_layer_sizes=(100,), early_stopping=True, random_state=12345, verbose=True)
